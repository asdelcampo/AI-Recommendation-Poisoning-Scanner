...
data/{dataset}.csv
    ...
    └─[5] Evaluation          eval.py
           Fixed benchmark: 8 labeled URLs (4 positive, 4 negative)
           Playwright fetch + all three detectors per URL
           Precision / Recall / F1 computed against ground truth
           output/eval/eval_results.json  — per-URL outcomes (JSON)
           output/eval/eval_summary.txt   — aggregate metrics
           output/eval/evidence/          — high-confidence forensic snapshots
```
...
`eval.py` is a fixed benchmark suite that measures the precision, recall, and F1 of the full detection pipeline against a labeled ground truth dataset. It transforms the research from "we scanned and found nothing" into "we built a detector with measurable accuracy and applied it at regional scale."

### Ground truth dataset

| URL | Expected | Label |
|---|---|---|
| llmrefs.com/blog/citemet-ai-share-buttons | positive | ai_poisoning_link + citemet |
| cite-met.com/blog/post/what-is-citemet | positive | citemet |
| metehan.ai/blog/citemet-ai-share-buttons-growth-hack-for-llms | positive | ai_poisoning_link + citemet |
| managednerds.com/seo/how-to-add-summarize-with-chatgpt-to-your-blog | positive | ai_poisoning_link |
| g2.com/articles/perplexity-vs-chatgpt | negative | clean |
| clickup.com/blog/perplexity-ai-vs-chatgpt | negative | clean |
| example.com | negative | clean |
| llmrefs.com (homepage only) | negative | FP stress test |

The four positive URLs are confirmed or strongly expected to carry CiteMET share buttons or raw poison-payload hrefs. The four negative URLs discuss AI platforms legitimately or serve as baselines with no poison payloads expected.

### Methodology

- All URLs fetched with `playwright_fetcher.py` (`fetch_with_playwright`) — static HTML is confirmed to produce false negatives for JS-rendered CiteMET.
- All three detectors run per page: `detect_ai_poisoning()`, `detect_citemet()`, `detect_hidden_text()`.
- URLs that time out or return errors are marked **SKIP** (excluded from metric counts) rather than counted as FN or FP.
- Any `confidence="high"` finding during eval is persisted to `output/eval/evidence/` as an HTML + JSON sidecar.

### Scoring

| Condition | Label |
|---|---|
| expected positive, ≥1 finding | TP |
| expected positive, 0 findings | FN |
| expected negative, 0 findings | TN |
| expected negative, ≥1 finding | FP |
...
The eval suite must pass (F1 > 0) before any future methodology change — new detector signal, keyword list edit, proximity window adjustment, or fetcher swap — is considered validated. A methodology change that degrades F1 relative to the prior run requires explicit documentation of the tradeoff.
...
Results are saved to `output/eval/eval_results.json` and `output/eval/eval_summary.txt`.
