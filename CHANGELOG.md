...
[2026-05-06] — Phase 8: Evaluation Suite

Added
- eval.py: ground truth evaluation suite — 8 labeled URLs (4 positive, 4 negative),
  all fetched via playwright_fetcher.py (JS-rendered); all three detectors run per URL;
  Precision / Recall / F1 computed; results saved to output/eval/eval_results.json and
  output/eval/eval_summary.txt; high-confidence findings persisted to output/eval/evidence/
Changed
- DOCUMENTATION.md: Architecture diagram updated to include eval.py as pipeline step [5]
- DOCUMENTATION.md: Evaluation section added with ground truth table, scoring methodology,
  and validation requirement (eval suite must pass before any future methodology change
  is accepted)
Fixed
- (none)
Issues
